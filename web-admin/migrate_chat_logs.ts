...
async function migrate() {
    ...
    console.log(`Found ${logs.length} legacy batched logs.`);
    ...
    for (const log of logs) {
        try {
            const rawProducts = log.product_name.split(',').map((s: string) => s.trim()).filter(Boolean);
            const inserts = [];
            ...
            for (const text of rawProducts) {
                ...
                const { id, created_at, updated_at, ...baseLog } = log;
            ...
            console.error(`Error migrating log ID ${log.id}:`, err);
            failCount++;
        }
    }
